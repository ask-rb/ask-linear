...
`0.1.5`. Never skip or jump patch numbers (`0.1.3` → `0.1.6` is wrong), even
...
This is an `ask-*` gem, so **every** release runs through **gemchain** from
the workspace root — never `rake release`, never a hand-run
`gem build` / `gem push`:

```bash
cd /Users/kaka/Code/ask-rb
gemchain guard ask-linear
gemchain update ask-linear <new-version> --dry-run
gemchain update ask-linear <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-linear.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/linear/version.rb` by gemchain, not by hand.
...
| `lib/ask/linear/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Several bug fixes ship together: one release, `0.1.3` → `0.1.4` (never
  `0.1.6`).
- New backwards-compatible API: `0.1.4` → `0.2.0`.
